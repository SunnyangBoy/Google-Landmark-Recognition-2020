...
from dataset import LandmarkDataset, get_test_df, get_transforms
...
def main():
    ...
    # get dataframe
    df, out_dim = get_test_df(args.kernel_type, args.data_dir, args.train_step)
    ...
    df_valid['filepath_flag'] = df_valid['id'].apply(lambda x: os.path.exists(os.path.join(args.data_dir, 'test', x[0], x[1], x[2], f'{x}.jpg')))
    delete = df_valid[df_valid['filepath_flag'] == False]
    df_valid = df_valid.drop(delete.index)
    
    df_valid['filepath'] = df_valid['id'].apply(lambda x: os.path.join(args.data_dir, 'test', x[0], x[1], x[2], f'{x}.jpg'))

    dataset_valid = LandmarkDataset(df_valid, 'train', 'val', transform=transforms_val)
    valid_loader = torch.utils.data.DataLoader(dataset_valid, batch_size=args.batch_size, num_workers=args.num_workers)

    # model
    model = ModelClass(args.enet_type, out_dim=out_dim, load_pretrained=False)
    model = model.cuda()
...
#                 optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    ...
    print('*********************** test result ***********************')
    content = time.ctime() + ' ' + f'valid loss: {(val_loss):.5f}, acc: {(acc_m):.6f}, gap: {(gap_m):.6f}.'
    print(content)
    print('\n')
    print('*********************** test finished ***********************')
